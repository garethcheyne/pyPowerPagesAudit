...
| `naming` | remediation helper | Proposes self-describing names for table permission records. |
...
```bash
python -m ppaudit naming --format "{table} [{privileges}] scope={scope} ({roles})"
```

Placeholders: `{table}` `{scope}` `{scope_chain}` `{root_scope}` `{parent}`
`{privileges}` `{privileges_compact}` `{privileges_long}` `{roles}` `{website}`
`{name}`. Add `--only-unclear` to leave names that already mention their table
and scope.

Where two records still produce the same name, the group is disambiguated by
the first tie-breaker that separates all of them — web role, then the scope its
parent chain resolves to, then the parent permission. Falling through all three
to a `#1`/`#2` counter is itself worth knowing: it means the records are
indistinguishable in table, scope, chain, privileges *and* roles, so one is
probably redundant.
...
```
pip install -r requirements.txt      # requests, xmltodict
```
...
ppaudit/
  ...
  naming.py      proposed naming convention for table permission records
